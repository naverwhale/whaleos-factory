  ...
  for component in ${components}; do
    ...
    case "${component}" in
      test_image_only)
        for i in $(seq 1 12); do
          install_payload "partition" "${json_url}" \
            "$(get_partition_dev "${dest}" "${i}")" \
            "${json_file}" "test_image.part${i}"
        done
        ;;
      release_image_only)
        for i in $(seq 1 12); do
          install_payload "partition" "${json_url}" \
            "$(get_partition_dev "${dest}" "${i}")" \
            "${json_file}" "release_image.part${i}"
        done
        ;;
      test_image)
        for mapping in "1 1" "4 2" "3 3" "12 12"; do
          from="${mapping% *}"
          to="${mapping#* }"
          install_payload "partition" "${json_url}" \
      ...
      release_image)
        ...
        # lsb-release.
        for mapping in "6 6" "7 7" "8 8" "9 9" "10 10" "11 11" \
          "4 4" "3 5"; do
          from="${mapping% *}"
          to="${mapping#* }"
          install_payload "partition" "${json_url}" \
